...
(function() {
    ...
    const State = {
        currentPlatform: (() => {
            ...
            if (
                ...
                host.endsWith("easychat.top") ||
                host.includes(".easychat.top")
            ) {
            ...
            if (
                ...
                (host.includes("x.com") && path.includes("/i/grok"))
            ) {
    ...
    const i18n = {
        languages: {
            zh: {
                ...
                exportCurrentJSON: "导出当前",
                exportAllConversations: "导出全部",
                branchMode: "多分支",
                includeImages: "含图像",
                enterFilename: "请输入文件名(不含扩展名):",
                untitledChat: "未命名对话",
                ...
                userWorkspace: "个人区",
                teamWorkspace: "工作区",
                manualWorkspaceId: "手动设置工作区ID",
                enterWorkspaceId: "请输入工作区ID (工作空间设置/工作空间 ID):",
                workspaceIdSaved: "工作区ID已保存!",
                tokenNotFound: "未找到访问令牌!",
                viewOnline: "预览对话",
                loadFailed: "加载失败: ",
            ...
            en: {
                ...
                exportCurrentJSON: "Export",
                exportAllConversations: "Save All",
                branchMode: "Branch",
                includeImages: "Images",
                enterFilename: "Enter filename (without extension):",
                ...
                manualUserId: "Customize UUID",
                enterUserId: "Organization ID (settings/account)",
                userIdSaved: "User ID saved!",
                workspaceType: "Workspace",
                userWorkspace: "Personal",
                teamWorkspace: "Team",
                manualWorkspaceId: "Set Workspace ID",
                enterWorkspaceId: "Enter Workspace ID(Workspace settings/Workspace ID):",
                workspaceIdSaved: "Workspace ID saved!",
        ...
        currentLang: localStorage.getItem("lyraExporterLanguage") ||
    ...
    const zipIcon =
        '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 11V9a7 7 0 0 0-7-7a7 7 0 0 0-7 7v2"></path><rect x="3" y="11" width="18" height="10" rx="2" ry="2"></rect></svg>';
    ...
    const ErrorHandler = {
        handle: (error, context, options = {}) => {
            ...
            if (showAlert) {
                const displayMsg =
                    userMessage || `${i18n.t("exportFailed")} ${errorMsg}`;
                alert(displayMsg);
            }
    ...
    const Utils = {
        ...
        sanitizeFilename: (name) =>
            name.replace(/[^a-z0-9\u4e00-\u9fa5]/gi, "_").substring(0, 100),
        ...
        setButtonLoading: (btn, text) => {
            ...
            safeSetInnerHTML(
                btn,
                `<div class="lyra-loading"></div> <span>${text}</span>`
            );
        },
    ...
    function extractGlobalCanvasContent() {
        ...
        try {
            const retryBtnGlobal = document.querySelector(
                "button.retry-without-tool-button"
            );
            if (retryBtnGlobal) {
        ...
        codeBlocks.forEach((block) => {
            ...
            const langAttr = block.querySelector("[data-lang]");
            const language = langAttr ?
                langAttr.getAttribute("data-lang") || "unknown" :
                "unknown";
            canvasData.push({
                type: "code",
                content: trimmed,
        ...
        const responseElements = document.querySelectorAll(
            "response-element, .model-response-text, .markdown"
        );
        responseElements.forEach((element) => {
            ...
            try {
                ...
                clone
                    .querySelectorAll("button.retry-without-tool-button")
                    ...
                    if (event.data && event.data.type === "LYRA_READY") {
                        ...
                        const dataToSend = {
                            ...
                            data: {
                                content: jsonData,
                                filename: filename ||
                                    `${State.currentPlatform}_export_${new Date()
                    .toISOString()
                    .slice(0, 10)}.json`,
                            },
                        };
    ...
    const ClaudeHandler = {
        ...
        addUI: (controlsArea) => {
            ...
            controlsArea.appendChild(
                Utils.createToggle(
                    i18n.t("branchMode"),
                    Config.TREE_SWITCH_ID,
                    treeMode
                )
            );
            ...
            controlsArea.appendChild(
                Utils.createButton(
                    ...
                    async (btn) => {
                        ...
                        try {
                            const includeImages =
                                document.getElementById(Config.IMAGE_SWITCH_ID) ? .checked ||
                                false;
                            const data = await ClaudeHandler.getConversation(
                                uuid,
                                includeImages
                            );
                            if (!data) throw new Error(i18n.t("fetchFailed"));
                            const jsonString = JSON.stringify(data, null, 2);
                            const filename = `claude_${
                data.name || "conversation"
              }_${uuid.substring(0, 8)}.json`;
                ...
                Utils.createButton(
                    ...
                    async (btn) => {
                        ...
                        const filename = prompt(
                            i18n.t("enterFilename"),
                            Utils.sanitizeFilename(`claude_${uuid.substring(0, 8)}`)
                        );
        ...
        getCurrentUUID: () =>
            window.location.pathname.match(/\/chat\/([a-zA-Z0-9-]+)/) ? . [1],
        ensureUserId: async () => {
        ...
        getAllConversations: async () => {
            ...
            try {
                const response = await fetch(
                    `${ClaudeHandler.getBaseUrl()}/api/organizations/${userId}/chat_conversations`
                );
        ...
        getConversation: async (uuid, includeImages = false) => {
            ...
            try {
                const treeMode =
                    document.getElementById(Config.TREE_SWITCH_ID) ? .checked || false;
                        ...
                        for (const key of fileArrays) {
                            if (Array.isArray(msg[key])) {
                                for (const file of msg[key]) {
                                    ...
                                    const imageUrl =
                                        file.preview_url || file.thumbnail_url || file.file_url;
                                    if (isImage && imageUrl && !file.embedded_image) {
                                        try {
                                            const fullUrl = imageUrl.startsWith("http") ?
                                                imageUrl :
                                                ClaudeHandler.getBaseUrl() + imageUrl;
        ...
        exportAll: async (btn, controlsArea) => {
            ...
            const promptMsg =
                i18n.currentLang === "zh" ?
                `${i18n.t("foundConversations")} ${totalCount} ${i18n.t(
              "conversations"
            ...
            try {
                const includeImages =
                    document.getElementById(Config.IMAGE_SWITCH_ID) ? .checked || false;
                let exported = 0;

                // 只导出最近的 exportCount 个对话
                const convsToExport = allConvs.slice(0, exportCount);
                console.log(
                    `Starting export of ${convsToExport.length} conversations (out of ${totalCount} total)`
                );

                const zipEntries = {};
                for (let i = 0; i < convsToExport.length; i++) {
                    const conv = convsToExport[i];
                    progress.textContent = `${i18n.t("gettingConversation")} ${i + 1}/${
            convsToExport.length
          }${includeImages ? i18n.t("withImages") : ""}`;
                    if (i > 0 && i % 5 === 0) {
                        await new Promise((resolve) =>
                            setTimeout(resolve, Config.TIMING.BATCH_EXPORT_YIELD)
                        );
                    } else if (i > 0) {
                    ...
                    try {
                        ...
                        if (data) {
                            ...
                            const filename = `claude_${conv.uuid.substring(
                0,
                8
              )}_${title}.json`;
                            zipEntries[filename] = fflate.strToU8(
                                JSON.stringify(data, null, 2)
                            );
                            exported++;
                        }
                ...
                const zipUint8 = fflate.zipSync(zipEntries, {
                    level: 1
                });
                const zipBlob = new Blob([zipUint8], {
                    type: "application/zip"
                });

                const zipFilename = `claude_export_${
          exportCount === totalCount ? "all" : "recent_" + exportCount
        }_${new Date().toISOString().slice(0, 10)}.zip`;
                Utils.downloadFile(zipBlob, zipFilename);
                alert(
                    `${i18n.t("successExported")} ${exported} ${i18n.t("conversations")}`
                );
            } catch (error) {
    ...
    const ChatGPTHandler = {
        ...
        extractImagesFromDOM: async (
            ...
            if (currentId !== conversationId) {
                console.log(
                    "[ChatGPT] Not current conversation, skipping DOM image extraction"
                );
                return {};
            }
            ...
            for (const group of messageGroups) {
                ...
                const userContainer = group.querySelector(
                ...
                if (userContainer) {
                    ...
                    const userImages = userContainer.querySelectorAll(
                        'img[src*="backend-api"], img[src*="files.oaiusercontent.com"], img[src*="oaiusercontent"]'
                    );
                    if (userImages.length > 0) {
                        ...
                        if (images.length > 0 && lastUserMessageId) {
                            if (!imageMap[lastUserMessageId])
                                imageMap[lastUserMessageId] = {};
                ...
                const assistantContainer = group.querySelector(
                ...
                addImages(
                    group.querySelectorAll(
                        'img[src*="estuary/content"], img[src*="estuary"]'
                    )
                );
                ...
                addImages(
                    group.querySelectorAll(
                        'img[alt*="生成"], img[alt*="Generated"], img[alt*="generated"]'
                    )
                );
                ...
                group.querySelectorAll("div").forEach((div) => {
                    ...
                    if (
                        classList.includes("imagegen") ||
                        classList.includes("image-gen")
                    ) {
                ...
                addImages(
                    group.querySelectorAll(
                        'img[aria-label*="图片"], img[aria-label*="image"]'
                    )
                );
                ...
                group
                    .querySelectorAll('[data-message-author-role="user"] img')
                    .forEach((img) => userImgSrcs.add(img.src));

                const uniqueImages = candidateImages.filter(
                    (img) => !userImgSrcs.has(img.src)
                );
                ...
                if (uniqueImages.length > 0) {
                    ...
                    for (const img of uniqueImages) {
                        ...
                        if (parent) {
                            ...
                            const parentFilter =
                                parentStyle.filter || parentStyle.webkitFilter || "";
                        ...
                        if (
                            img.getAttribute("aria-busy") === "true" ||
          ...
          response.status
                ...
                if (response.status === 404) {
                    const currentMode =
                        ...
                        i18n.t("teamWorkspace") :
                        i18n.t("userWorkspace");
                    const suggestMode =
                        State.chatgptWorkspaceType === "team" ?
                        i18n.t("userWorkspace") :
                        i18n.t("teamWorkspace");
            ...
            if (includeImages) {
                const imageMap = await ChatGPTHandler.extractImagesFromDOM(
                    conversationId,
                    includeImages,
                    token
                );
                    ...
                    for (const [messageId, images] of Object.entries(imageMap)) {
                        ...
                        if (nodeId && data.mapping[nodeId]) {
                            ...
                            if (images.assistant_generated) {
                                data.mapping[nodeId].lyra_images.assistant_generated =
                                    images.assistant_generated;
                            }
                        }
                    }
                }
        ...
        previewConversation: async () => {
            ...
            try {
                ...
                const data = await ChatGPTHandler.getConversation(
                    conversationId,
                    includeImages
                );
                const jsonString = JSON.stringify(data, null, 2);
                const filename = `chatgpt_${
          data.title || "conversation"
        }_${conversationId.substring(0, 8)}.json`;
                await LyraCommunicator.open(jsonString, filename);
            } catch (error) {
                ErrorHandler.handle(error, "Preview conversation", {
                    userMessage: `${i18n.t("loadFailed")} ${error.message}`,
        ...
        exportCurrent: async (btn) => {
            ...
            try {
                ...
                const filename = prompt(
                    i18n.t("enterFilename"),
                    data.title || i18n.t("untitledChat")
                );
                if (!filename) {
                    Utils.restoreButton(btn, original);
                    return;
                ...
                Utils.downloadJSON(
                    JSON.stringify(data, null, 2),
                    `${Utils.sanitizeFilename(filename)}.json`
                );
            } catch (error) {
                ErrorHandler.handle(error, "Export conversation");
            } finally {
            ...
            convsToExport.length
                    ...
                    try {
                        ...
                        const shouldExtractImages =
                            includeImages && conv.id === currentConvId;
                        const data = await ChatGPTHandler.getConversation(
                            conv.id,
                            shouldExtractImages
                        );
                        if (data) {
                            ...
                            const filename = `chatgpt_${conv.id.substring(
                0,
                8
              )}_${title}.json`;
                            zipEntries[filename] = fflate.strToU8(
                ...
                const zipFilename = `chatgpt_export_${
          exportCount === totalCount ? "all" : "recent_" + exportCount
        }_${new Date().toISOString().slice(0, 10)}.zip`;
                Utils.downloadFile(zipBlob, zipFilename);
                alert(
        ...
        addUI: (controls) => {
            ...
            toggleInput.addEventListener("change", (e) => {
                ...
                toggleLabel.textContent = e.target.checked ?
                    ...
                    i18n.t("userWorkspace");
                console.log(
                    "[ChatGPT] Workspace type changed to:",
                    State.chatgptWorkspaceType
                );
                UI.recreatePanel();
            });
        ...
        addButtons: (controls) => {
            controls.appendChild(
                Utils.createButton(`${previewIcon} ${i18n.t("viewOnline")}`, () =>
        ...
        try {
            ...
            if (url.startsWith("blob:")) {
                ...
                try {
                    ...
                    console.warn(
                        "[Grok] Canvas method failed, using GM_xmlhttpRequest fallback:",
                        canvasError.message
                    );
    ...
    const GrokHandler = {
        ...
        getConversation: async (conversationId) => {
            try {
                ...
                if (!nodeResponse.ok)
                    throw new Error(
                        `Failed to get response nodes: ${nodeResponse.status}`
                    );
                ...
                if (!loadResponse.ok)
                    throw new Error(`Failed to load responses: ${loadResponse.status}`);
                ...
                const processedResponses = (conversationData.responses || [])
                    ...
                    .map((r) => {
                            ...
                            try {
                                r.cardAttachmentsJson.forEach((cardStr) => {
                                    ...
                                    if (card.cardType === "citation_card" && card.url) {
                                        const searchResult = r.webSearchResults.find(
                                            (sr) => sr.url === card.url
                                        );
                                        citations.push({
                                            id: card.id,
                        ...
                        if (r.cardAttachmentsJson)
                            processed.cardAttachmentsJson = r.cardAttachmentsJson;
                        if (r.imageAttachments)
                            processed.imageAttachments = r.imageAttachments;
                        if (r.fileAttachments)
                            processed.fileAttachments = r.fileAttachments;
                ...
                if (State.includeImages) {
                    ...
                    const messageContainers =
                        document.querySelectorAll('[id^="response-"]');
                    ...
                    for (const container of messageContainers) {
                        ...
                        const generatedImgs = container.querySelectorAll(
                            '[data-testid="image-viewer"] img[src*="assets.grok.com"]'
                        );
                        ...
                        if (capturedImages.length > 0) {
                            ...
                            console.log(
                                `[Grok] Captured ${capturedImages.length} AI-generated image(s) for response ${responseId}`
                            );
                        }
                    }
                    ...
                    const allUserImages = document.querySelectorAll(
                        'figure img[src*="assets.grok.com"][src*="preview-image"]'
                    );
                    ...
                    for (const img of allUserImages) {
                        ...
                        try {
                            ...
                            if (imageData) {
                                ...
                                const humanResponses = processedResponses.filter(
                                    (r) =>
                                    r.sender === "human" &&
                                    r.fileAttachments &&
                                ...
                                if (humanResponses.length > 0) {
                                    const targetResponse =
                                        humanResponses[humanResponses.length - 1];
                                    ...
                                    console.log(
                                        `[Grok] Captured user-uploaded image for response ${targetResponse.responseId}`
                                    );
                                } else {
                                    console.warn(
                                        "[Grok] No human messages with file attachments found to attach user image"
                                    );
                                }
                            }
                        } catch (e) {
                ...
                return {
                    ...
                    conversationTree: {
                        ...
                        rootNodeId: responseNodes.find((n) => !n.parentResponseId) ? .responseId ||
                            null,
                    },
            ...
            controls.appendChild(
                Utils.createButton(
                    ...
                    async (btn) => {
                        ...
                        const filename = prompt(
                            i18n.t("enterFilename"),
                            Utils.sanitizeFilename(`grok_${conversationId.substring(0, 8)}`)
                        );
                        if (!filename ? .trim()) return;
                        const original = btn.innerHTML;
                        Utils.setButtonLoading(btn, i18n.t("exporting"));
        ...
        exportAll: async (btn, controlsArea) => {
            ...
            try {
                ...
                const convsToExport = allConvs.slice(0, exportCount);
                console.log(
                    `[Grok] Starting export of ${convsToExport.length} conversations (out of ${totalCount} total)`
                );
            ...
            convsToExport.length
                    ...
                    try {
                        ...
                        if (data) {
                            const title = Utils.sanitizeFilename(
                                data.title || conv.conversationId
                            );
                            const filename = `grok_${conv.conversationId.substring(
                0,
                8
              )}_${title}.json`;
                            zipEntries[filename] = fflate.strToU8(
                        ...
                        console.error(
                            `[Lyra] Failed to process ${conv.conversationId}:`,
                            error
                        );
                    }
                }
                ...
                const zipFilename = `grok_export_${
          exportCount === totalCount ? "all" : "recent_" + exportCount
        }_${new Date().toISOString().slice(0, 10)}.zip`;
                Utils.downloadFile(zipBlob, zipFilename);
                alert(
    ...
    const VersionTracker = {
        ...
        getImageHashKey: (img) =>
            img ? `${img.size}-${img.format}-${img.data.substring(0, 100)}` : null,
        ...
        getOrFetchImage: async (imgElement, retries = 3) => {
            ...
            if (
                ...
                return null;
            if (VersionTracker.imageCache.has(url))
                return VersionTracker.imageCache.get(url);
        ...
        areImageListsEqual: (a, b) => {
            ...
            return a.every(
                (img, i) => img.size === b[i].size && img.data === b[i].data
            );
        },
        ...
        handleUser: (turnId, text, images = []) => {
            ...
            const isImagesSame = VersionTracker.areImageListsEqual(
                lastImages,
                images
            );
        ...
        handleAssistant: (turnId, domText, images = []) => {
            ...
            if (
                last ? .userVersion === userVersion &&
                last ? .text === text &&
                VersionTracker.areImageListsEqual(lastImages, images)
            ) {
                t.assistantPendingSince = now;
                return;
            }
        ...
        scanOnce: async () => {
            ...
            try {
                ...
                for (const turn of turns) {
                    ...
                    if (includeImages) {
                        const userImgEls = turn.querySelectorAll(
                            "user-query img, user-query-file-preview img, .file-preview-container img"
                        );
                        // 只获取 message-content 内的图片，排除 model-thoughts
                        const modelContent = turn.querySelector(
                            "model-response message-content"
                        );
                        const modelImgEls = modelContent ? .querySelectorAll("img") || [];

                        if (userImgEls.length)
                            userImages = (
                                await Promise.all(
                                    [...userImgEls].map((i) => VersionTracker.getOrFetchImage(i))
                                )
                            ).filter(Boolean);
                        if (modelImgEls.length)
                            assistantImages = (
                                await Promise.all(
                                    [...modelImgEls].map((i) => VersionTracker.getOrFetchImage(i))
                                )
                            ).filter(Boolean);
                    }

                    VersionTracker.handleUser(
                        id,
                        VersionTracker.getUserText(turn),
                        userImages
                    );
                    VersionTracker.handleAssistant(
                        id,
                        VersionTracker.getAssistantText(turn),
                        assistantImages
                    );
                }
            } finally {
        ...
        getUserText: (turn) =>
            (
                turn.querySelector(
                    "user-query .query-text, .query-text-line, [data-user-text]"
                ) ? .innerText || ""
            ).trim(),
        ...
        getAssistantText: (turn) => {
            ...
            clone
                .querySelectorAll(
                    "button.retry-without-tool-button, model-thoughts, .model-thoughts, .thoughts-header"
                )
                .forEach((b) => b.remove());
        ...
        buildVersionedData: (title) => {
            ...
            for (const id of order) {
                ...
                const mapVersions = (versions, imgMap) =>
                    versions
                ...
                result.push({
                    ...
                    human: t.userVersions.length ?
                        {
                            versions: mapVersions(t.userVersions, t.userImages)
                        } :
                        null,
                    assistant: t.assistantVersions.length ?
                        {
                            versions: mapVersions(t.assistantVersions, t.assistantImages)
                        } :
                        null,
                });
            }
    ...
    window.lyraGeminiExport = (title) =>
        VersionTracker.buildVersionedData(title || "Gemini Chat");
    ...
    function fetchViaGM(url) {
        return new Promise((resolve, reject) => {
            if (typeof GM_xmlhttpRequest === "undefined") {
                fetch(url)
                    .then((r) =>
                        r.ok ? r.blob() : Promise.reject(new Error(`Status: ${r.status}`))
                    )
                    .then(resolve)
                    .catch(reject);
                return;
        ...
        for (const turn of document.querySelectorAll("ms-chat-turn")) {
            ...
            if (userEl) {
                ...
                if (includeImages) {
                    ...
                    turnData.images = (
                        await Promise.all([...imgs].map(processImageElement))
                    ).filter(Boolean);
                }
            } else if (modelEl) {
                ...
                chunks.forEach((chunk) => {
                    ...
                    if (cmark) {
                        ...
                        if (md) texts.push(md);
                        if (includeImages)
                            [...cmark.querySelectorAll("img")].forEach((i) =>
                                imgPromises.push(processImageElement(i))
                            );
                    }
                });
            ...
            if (
                turnData.type !== "unknown" &&
                (turnData.text || turnData.images.length)
            ) {
    ...
    const ScraperHandler = {
                ...
                extractData: async (includeImages = true) => {
                    ...
                    for (const container of turns) {
                        const userEl = container.querySelector(
                            "user-query .query-text, .query-text-line"
                        );
                        // 严格只从 message-content 获取内容
                        const messageContent = container.querySelector("message-content");
                        const modelEl = messageContent ? .querySelector(
                            ".markdown-main-panel"
                        );
                        ...
                        let userImages = [],
                            modelImages = [];
                        if (includeImages) {
                            const uImgs = container.querySelectorAll(
                                "user-query img, user-query-file-preview img, .file-preview-container img"
                            );
                            // 只从 message-content 获取图片
                            const mImgs = messageContent ? .querySelectorAll("img") || [];
                            userImages = (
                                await Promise.all([...uImgs].map(processImageElement))
                            ).filter(Boolean);
                            modelImages = (
                                await Promise.all([...mImgs].map(processImageElement))
                            ).filter(Boolean);
                        }
                ...
                extractData: async (includeImages = true) => {
                    ...
                    for (const turn of document.querySelectorAll(
                        ...
                        let question =
                            turn
                            .querySelector(
                                "chat-message .from-user-container .message-text-content"
                            ) ?
                            .innerText.trim() || "";
                        ...
                        let answer = "";
                        const answerEl = turn.querySelector(
                            "chat-message .to-user-container .message-text-content"
                        );
                        if (answerEl) {
                            const parts = [];
                            answerEl
                                .querySelectorAll("labs-tailwind-structural-element-view-v2")
                                .forEach((el) => {
                                    let line =
                                        el.querySelector(".bullet") ? .innerText.trim() + " " || "";
                                    ...
                                    if (para) {
                                        ...
                                        para.childNodes.forEach((n) => {
                                            if (n.nodeType === Node.TEXT_NODE) text += n.textContent;
                                                ...
                                                !n.querySelector ? .(".citation-marker")
                                            ) {
                                                text += n.classList ? .contains("bold") ?
                                                    `**${n.innerText}**` :
                                ...
                                await Promise.all(
                                    [
                                        ...turn.querySelectorAll(
                                            "chat-message .from-user-container img"
                                        ),
                                    ].map(processImageElement)
                                )
                            ).filter(Boolean);
                            modelImages = (
                                await Promise.all(
                                    [
                                        ...turn.querySelectorAll(
                                            "chat-message .to-user-container img"
                                        ),
                                    ].map(processImageElement)
                                )
                            ).filter(Boolean);
                ...
                extractData: async (includeImages = true) => {
                    ...
                    for (const item of sorted) {
                        if (item.type === "user") {
                            ...
                            lastHuman.text =
                                (lastHuman.text ? lastHuman.text + "\n" : "") + item.text;
                            if (item.images ? .length) lastHuman.images.push(...item.images);
                        } else if (item.type === "model") {
                            const human = {
                                text: lastHuman ? .text || "[No preceding user prompt found]",
                            };
                    ...
                    if (lastHuman) {
                        ...
                        paired.push({
                            human,
                            assistant: {
                                text: "[Model response is pending]"
                            },
                        });
                    }
                    return paired;
        ...
        buildConversationJson: async (platform, title) => {
            ...
            if (
                platform === "gemini" &&
                document.getElementById(Config.CANVAS_SWITCH_ID) ? .checked
            ) {
        ...
        addButtons: (controlsArea, platform) => {
            ...
            if (platform === "gemini") {
                controlsArea.appendChild(
                    createToggle(
                        i18n.t("versionTracking") || "版本追踪",
                        Config.CANVAS_SWITCH_ID,
                        State.includeCanvas,
                        (e) => {
            ...
            if (platform !== "notebooklm") {
                controlsArea.appendChild(
                    createActionBtn(previewIcon, "viewOnline", async (btn) => {
                        ...
                        let progress =
                            platform === "aistudio" ?
                            Utils.createProgressElem(controlsArea) :
                            null;
                        if (progress) progress.textContent = i18n.t("loading");
                        try {
                            const json = await ScraperHandler.buildConversationJson(
                                platform,
                                title
                            );
                            const filename = `${platform}_${Utils.sanitizeFilename(
                title
              )}_${new Date().toISOString().slice(0, 10)}.json`;
                            await LyraCommunicator.open(
                                JSON.stringify(json, null, 2),
                                filename
                            );
                        } catch (e) {
                            ErrorHandler.handle(e, "Preview conversation", {
                                userMessage: `${i18n.t("loadFailed")} ${e.message}`,
                            });
                        } finally {
                            Utils.restoreButton(btn, original);
                            progress ? .remove();
            ...
            controlsArea.appendChild(
                createActionBtn(exportIcon, "exportCurrentJSON", async (btn) => {
                    const title = handler.getTitle();
                    if (!title) return;
                    const original = btn.innerHTML;
                    Utils.setButtonLoading(btn, i18n.t("exporting"));
    ...
    const UI = {
        injectStyle: () => {
            ...
            console.log(
                "[Lyra] CSS variable --lyra-button-color set to:",
                buttonColor
            );
        ...
        createPanel: () => {
            if (document.getElementById(Config.CONTROL_ID) || State.panelInjected)
                return false;
            ...
            if (
                State.currentPlatform === "notebooklm" ||
                State.currentPlatform === "gemini"
            ) {
            ...
            safeSetInnerHTML(
                toggle,
                State.isPanelCollapsed ? collapseIcon : expandIcon
            );
    ...
    const init = () => {
        ...
        const initPanel = () => {
            UI.createPanel();
            if (
                State.currentPlatform === "claude" ||
                State.currentPlatform === "chatgpt" ||
                State.currentPlatform === "grok"
            ) {
