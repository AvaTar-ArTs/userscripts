// --- Suno Advanced Extractor (FIXED for Saved HTML) ---
// Advanced extraction with lyrics, side panel, retry logic, and iframe fallback
// NOW WORKS WITH SAVED HTML FILES!
...
(function() {
    ...
    async function extractSunoAdvanced(options = {}) {
        ...
        // Detect if we're on a saved HTML file
        const isSavedFile = window.location.protocol === 'file:' || window.location.href.startsWith('file://');
        if (isSavedFile) {
            console.log('📄 Detected saved HTML file - using DOM traversal mode');
        }

        // STEP 1: AUTO-SCROLL TO LOAD CONTENT (skip on saved files)
        let nscroll = 0,
            lastCount = 0,
            noChange = 0;
        if (!isSavedFile) {
            ...
            // On saved files, just count what's already there
            const anchors = Array.from(document.querySelectorAll('a[href*="/song/"]'));
            const unique = new Set(anchors.map(a => (a.href.match(/\/song\/([a-f0-9-]{36})/) || [])[1]).filter(Boolean));
            lastCount = unique.size;
            console.log(`📄 Found ${lastCount} unique song links in saved HTML.`);
        }
                ...
                try {
                    ...
                    if (!isSavedFile) {
                        try {
                            ...
                            if (res.ok) {
                                ...
                                const summary = clean(summaryEl ? summaryEl.textContent : (metaDesc ? metaDesc.content : ''));
                                const authorEl = doc.querySelector(CONFIG.SELECTORS.AUTHOR);
                                const author = clean(authorEl ? authorEl.textContent : '');
                                const tagEls = doc.querySelectorAll(CONFIG.SELECTORS.TAGS || '');
                                ...
                                const audio = audioNode ? (audioNode.src || audioNode.content) : '';
                                if (lyrics || summary) {
                                    const song = {
                                        id,
                                        href,
                            ...
                            console.debug(`   Fetch attempt failed for ${id}:`, fetchErr.message);
                        }
                    }
                    ...
                    if (isSavedFile) {
                        ...
                        if (songLink) {
                            ...
                            let container = songLink.closest('[data-clip-id], [data-testid="song-row"], .clip-row, div');
                            ...
                            for (const selector of lyricsSelectors) {
                                const lyricsEl = container ? container.querySelector(selector) : null;
                            ...
                            // If no lyrics found, search for divs with verse markers
                            if (!lyrics) {
                                const allDivs = document.querySelectorAll('div');
                                for (const div of allDivs) {
                                    const text = div.textContent.trim();
                                    // Look for divs with verse markers and substantial text
                                    if (text.length > 100 && (
                                            text.includes('[Verse') ||
                                            text.includes('[Chorus') ||
                                            text.includes('[Bridge')
                                        )) {
                                        // Check if it's near the song link
                                        let nearby = false;
                                        let checkEl = div;
                                        for (let j = 0; j < 10; j++) {
                                            if (checkEl.querySelector(`a[href*="/song/${id}"]`)) {
                                                nearby = true;
                                                break;
                                            }
                            ...
                            for (const selector of summarySelectors) {
                                const summaryEl = container ? container.querySelector(selector) : null;
                                if (summaryEl) {
                                    summary = clean(summaryEl.textContent || (summaryEl.content || ''));
                                    if (summary) break;
                                }
                            }
                            ...
                            const authorEl = container ?
                                (container.querySelector('a[href^="/@"]') ||
                                    (songLink.closest('div') ? songLink.closest('div').querySelector('a[href^="/@"]') : null)) :
                                null;
                            const author = clean(authorEl ? authorEl.textContent : '');

                            // Get tags
                            const tagEls = container ?
                                container.querySelectorAll('a[href*="/style/"], [data-testid="tags"] span, .tags span') : [];
                            const tags = Array.from(tagEls).map(n => clean(n.textContent)).filter(Boolean).join(', ');

                            // Get title
                            const title = clean(el.getAttribute('title') || el.textContent.trim() || songLink.textContent.trim());

                            if (lyrics || summary || title) {
                                const song = {
                                    id,
                                    href: href.startsWith('http') ? href : `https://suno.com${href}`,
                                    title,
                                    author,
                                    tags,
                                    lyrics,
                                ...
                                progress.processed[id] = song;
                                sessionStorage.setItem(CONFIG.RESUME_KEY, JSON.stringify(progress));
                                console.log(`✅ [${i+1}] ${song.title || id} — saved HTML (lyrics: ${!!lyrics})`);
                                ok = true;
                                break;
                            }
                        }
                        ...
                        iframe.src = href.startsWith('http') ? href : `https://suno.com${href}`;
                        document.body.appendChild(iframe);
                        await new Promise(resolve => setTimeout(resolve, CONFIG.PANEL_WAIT_IFRAME / 10));
                        let idoc = null;
                        try {
                        ...
                        if (idoc) {
                            ...
                            summary = clean(summaryEl ? summaryEl.textContent : (metaDesc ? metaDesc.content : ''));
                            ...
                            tags = Array.from(tagEls).map(n => n.textContent.trim()).filter(Boolean).join(', ');
    ...
    function exportSunoSongsData(songs, prefix = 'suno-adv') {
        ...
        const fields = [
            'id', 'title', 'author', 'tags', 'lyrics', 'summary', 'audio', 'scrapedAt', 'error', 'source'
        ...
        songs.slice(0, 10).forEach((s, i) => {
            console.log(`   ${i + 1}. ${s.title}${s.author ? ' - ' + s.author : ''} ${s.tags ? '- ' + s.tags : ''}`);
        });
    ...
    console.log('✅ Suno Advanced Extractor loaded (Fixed for saved HTML)');
    console.log('  > Run: await sunoExtractAdvanced()');
    console.log('  > Works on both live sites and saved HTML files!');
})();